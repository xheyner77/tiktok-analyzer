...
export function classifyAnalysisTransparency(result: Partial<AnalysisResult> | null | undefined): AnalysisTransparencyState {
  ...
  if (mode === 'demo') {
    return transparencyState('demo', result, 'Donnees de demonstration : ce diagnostic ne doit pas etre lu comme une analyse reelle de ta video.');
  }

  if (mode === 'fallback' || result?.analyzerMeta?.isFallback) {
    return transparencyState('fallback', result, 'Analyse degradee : une erreur technique a force un diagnostic prudent.');
  }

  if (mode === 'metadata' || result?.observedStatsSource === 'manual' || result?.observedPerformanceEstimated) {
    return transparencyState('estimated', result, 'Analyse estimee : les recommandations reposent sur des metadata, une URL ou des metriques non verifiees.');
  }

  if (!result?.analyzerMeta && !hasUploadFrames && !hasVerifiedStats) {
    return transparencyState('estimated', result, 'Analyse ancienne ou incomplete : Viralynz ne peut pas garantir que tous les signaux etaient observes.');
  }

  if (!hasUploadFrames && !hasVerifiedStats) {
    return transparencyState('estimated', result, 'Analyse estimee : aucun signal video ou statistique verifie ne permet un benchmark reel.');
  }

  if (hasWarnings || hasMissingSignals || (typeof confidenceScore === 'number' && confidenceScore < 70)) {
    return transparencyState('partial', result, 'Analyse partielle : certains signaux manquent, les scores doivent etre lus comme des hypotheses.');
  }
