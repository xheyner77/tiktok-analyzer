...
-- Legacy aliases are accepted only for existing rows: creator -> starter, scale/elite -> lifetime when paid.
...
AS $$
  SELECT CASE
    ...
    WHEN p_plan IN ('lifetime', 'scale') THEN 1000
    WHEN p_plan = 'elite'
      AND (
        p_subscription_status IN ('active', 'trialing', 'lifetime')
