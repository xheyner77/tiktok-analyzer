...
function statsSourceLabel(source: AnalysisResult['observedStatsSource']) {
  ...
  if (source === 'manual') return 'Source : saisie manuelle non vérifiée';
                ...
                {(observedSignals.length > 0 || hypothesisSignals.length > 0) && (
                  <div className="mt-3 grid gap-2 sm:grid-cols-2">
                    {observedSignals.length > 0 && (
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-[0.18em] text-amber-100/55">Observé</p>
                        <p className="mt-1 text-[11px] leading-snug text-amber-50/75">{observedSignals.join(' · ')}</p>
                      </div>
                    )}
                    {hypothesisSignals.length > 0 && (
                      <div>
                        <p className="text-[9px] font-black uppercase tracking-[0.18em] text-amber-100/55">Hypothèses IA</p>
          ...
          <div className="px-7 sm:px-10 py-8 text-center sm:text-left">
            ...
            {transparency.canShowRealBenchmark ? (
              ...
              <div className="max-w-lg rounded-2xl border border-white/[0.07] bg-white/[0.035] px-4 py-3 mx-auto sm:mx-0">
                <p className="text-[11px] font-bold text-gray-300">Benchmark réel indisponible</p>
                <p className="mt-1 text-[11px] leading-snug text-gray-500">
                  Les scores restent utiles pour décider quoi couper ou réécrire, mais Viralynz ne les présente pas comme un classement TikTok vérifié.
                </p>
              </div>
            )}
