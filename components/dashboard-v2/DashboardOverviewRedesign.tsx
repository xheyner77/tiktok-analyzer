...
function Header({ dashboard }: { dashboard: DashboardData }) {
  ...
  const quotaLabel = remaining === null ? 'Analyses illimitées' : `${remaining} analyse${remaining > 1 ? 's' : ''} restante${remaining > 1 ? 's' : ''}`;
    ...
    <header className="pt-[max(18px,env(safe-area-inset-top))] min-[1024px]:pt-0">
      ...
      <div className="mt-7 min-[1024px]:mt-0 min-[1024px]:flex min-[1024px]:items-end min-[1024px]:justify-between min-[1024px]:gap-8">
        <div>
          ...
          <p className="mt-2 max-w-[520px] text-[0.98rem] font-medium leading-6 text-slate-300/82 min-[1024px]:text-[1rem]">Tes meilleures actions IA du jour, sans métrique inventée.</p>
        </div>
...
function HeroCard({ dashboard, states }: { dashboard: DashboardData; states: DashboardData['states'] }) {
  ...
  const primaryLabel = hasAnalysis ? 'Préparer la V2' : hero.ctaLabel;
    ...
    <section className={cn(card, 'mt-5 border-cyan-200/14 p-5 min-[1024px]:mt-0 min-[1024px]:min-h-[360px] min-[1024px]:p-7')}>
      ...
      <div className="relative max-w-[310px] min-[1024px]:max-w-[580px]">
        ...
        <p className="mt-1 text-[0.78rem] font-black uppercase tracking-[0.16em] text-slate-500">
          {score === null ? 'Score disponible après analyse' : 'Score de repost'}
        </p>

        <div className="mt-5 max-w-[540px]">
          <p className="text-[0.82rem] font-black uppercase tracking-[0.16em] text-cyan-200/72">Meilleure opportunité</p>
          <h2 className="mt-1 text-[1.24rem] font-black leading-[1.1] tracking-[-0.025em] text-white min-[1024px]:text-[1.55rem]">
            {hasAnalysis ? hero.title : 'Aucune opportunité détectée sans analyse'}
          </h2>
...
function KpiBoard({ kpis }: { kpis: DashboardData['overview']['kpis'] }) {
  return (
    <section className={cn(card, 'mt-4 p-4 min-[1024px]:mt-0 min-[1024px]:p-5')}>
      <SectionTitle title="Signaux clés" eyebrow="Données disponibles" />
    ...
    <section className="mt-5 min-[1024px]:mt-0">
      ...
      {states.hasAnalyses && analyses.length > 0 ? (
        ...
        <div className={cn(card, 'p-5 text-left')}>
          ...
          <p className="mt-2 text-[0.86rem] leading-5 text-slate-400">Viralynz attend une vraie vidéo avant d’afficher scores, vues ou décisions de repost.</p>
          <Link href="/dashboard/analyze" className={cn(primaryCta, 'mt-4 h-11')}>Analyser une vidéo</Link>
        </div>
      )}
    </section>
  );
    ...
    <section className={cn(card, 'mt-5 border-violet-200/16 p-5 min-[1024px]:mt-0 min-[1024px]:p-6')}>
      ...
      <div className="relative">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[0.7rem] font-black uppercase tracking-[0.17em] text-violet-100/74">{block.available ? 'Copilote de repost' : 'Aucune donnée inventée'}</p>
        </div>
        ...
        <Link href={href} className={cn(secondaryCta, 'mt-5 h-11 px-4')}>{block.available ? 'Transformer en V2' : 'Analyser une vidéo'}</Link>
      </div>
    </section>
  );
}
    ...
    <section className="mt-5 min-[1024px]:mt-0">
      ...
      {visibleHooks.length > 0 ? (
          ...
          {visibleHooks.map((hook, index) => (
            <Link key={hook.id} href={hook.href} className={cn(mutedCard, 'flex min-h-[82px] items-center gap-3 p-3 transition hover:border-violet-200/18 hover:bg-white/[0.05] active:scale-[0.99]')}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[0.72rem] font-black uppercase tracking-[0.1em] text-slate-500">{hook.category || 'Angle à tester'}</p>
              </div>
              <Icon name="chevron" className="h-[18px] w-[18px] shrink-0 text-slate-500" />
            </Link>
          ))}
        ...
        <Link href="/dashboard/hooks" className={cn(card, 'flex min-h-[92px] items-center gap-3 p-4 transition active:scale-[0.99]')}>
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[0.95rem] font-black text-white">Aucun hook généré</p>
            <p className="mt-1 text-[0.82rem] leading-5 text-slate-400">Génère des hooks depuis une analyse ou une idée à tester.</p>
          </div>
          <Icon name="chevron" className="h-[18px] w-[18px] shrink-0 text-slate-500" />
        </Link>
      )}
    ...
    <Link href="/dashboard/settings" className={cn(card, 'mt-5 flex min-h-[104px] items-center gap-3 p-4 transition active:scale-[0.99] min-[1024px]:mt-0')}>
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded-[7px] border border-emerald-300/16 bg-emerald-300/10 px-2 py-0.5 text-[0.62rem] font-black uppercase tracking-[0.08em] text-emerald-100">Connecté</span>
        </div>
        <p className="mt-1 line-clamp-1 text-[0.78rem] font-semibold text-slate-400">{connection.lastSyncAt ? dashboardSyncLabel(connection.lastSyncAt) : 'Dernière sync non disponible'}</p>
      </div>
      <Icon name="chevron" className="h-[18px] w-[18px] text-slate-500" />
    </Link>
  );
...
function QuickActions() {
  ...
  return (
    <section className="mt-5 min-[1024px]:mt-0">
      <SectionTitle title="Raccourcis" eyebrow="Accès rapides" />
...
function PermissionBanner({
  ...
  return (
    <section className={cn(card, 'mt-4 flex items-start gap-3 border-amber-200/14 p-4 min-[1024px]:mt-0')}>
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[0.92rem] font-black text-white">Permissions TikTok à vérifier</p>
        <p className="mt-1 text-[0.8rem] leading-5 text-slate-400">Certaines métriques restent masquées tant que les scopes vidéo ne sont pas disponibles.</p>
      </div>
